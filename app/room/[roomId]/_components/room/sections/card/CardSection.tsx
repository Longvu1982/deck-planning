    ...
    <div className="flex items-center justify-center fixed left-0 right-0 bottom-10 h-40">
      {selectValues.map((item, index) => {
        ...
        return (
          <div
            key={item}
            onClick={() => onSelect(item, userIndex)}
            className={cn(
              "cursor-pointer select-none h-40 w-24 -ml-2 rounded-md flex items-center justify-center text-white shadow-lg transition-all",
